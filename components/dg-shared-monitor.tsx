...
export function DgSharedMonitor({ tables, connected, message, updatedAt, onFocusTable, cardColumns, onCardColumnsChange }: {
  tables: TableInfo[]; connected: boolean; message: string; updatedAt: string; onFocusTable?: (table: TableInfo) => void;
  cardColumns: CardColumns; onCardColumnsChange: (value: CardColumns) => void;
}) {
  return <section className="overflow-hidden rounded-2xl border border-[#86632f]/35 bg-[#0d0b08]/92">
    ...
    <div className={`grid w-full min-w-0 gap-3 bg-transparent p-2 ${cardGridColumns[cardColumns]}`}>
      {tables.map(table => <BaccaratTableCard key={table.id} table={table} connected={connected} platformLabel="DG" onFocusTable={onFocusTable} />)}
    </div>
  </section>;
}
